...
//        newWidgetFile =  FileUtils.createNewFile(widgetPath);
        try {
            ...
            if(FileUtils.isExists(imageDirectory)){
                ...
                EditImageExecute.execute(new File(imageDirectory));
            }else{
        ...
        try {
            if(FileUtils.isExists(widgetPath)){
                Log.appendInfo("开始场景文件>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
                EditTools.getWidgets(new File(widgetPath));//得到文件的所有控件
            }else{
            }
        } catch (Exception e) {
            e.printStackTrace();
        ...
        try {
            ...
            if(FileUtils.isExists(sceneDirectory)){
                Log.appendInfo("开始场景文件>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
                EditSceneExecute.start(new File(sceneDirectory));
            }else{
